...
functions: f1/4, g/4, f2/5, f3/7, resize/2
...
dnl Generate_JW_ResDCommitment(KeyboardDisplay,NoInputNoOutput)
dnl Generate_JW_ResDCommitment(NoInputNoOutput,KeyboardDisplay)
...
dnl Generate_JW_InitDSendNonce(KeyboardDisplay,NoInputNoOutput)
dnl Generate_JW_InitDSendNonce(NoInputNoOutput,KeyboardDisplay)


rule JW_ResDSendNonce [color=#FFF68F]:
    let
        Vb = g(DHpkI,'g'^~skR,Na,~Nb)
    in
    [
        JW_State_Res_Sent_Commitment(ResD,MacAddI,IOCapabilityI,OOBflagI,MITMI,MacAddR,IOCapabilityR,OOBflagR,MITMR,~skR,DHpkI,DHKeyR,ra,rb,~Nb),
...
dnl Generate_NC_ResDCommitment(KeyboardDisplay,DisplayYesNo)
dnl Generate_NC_ResDCommitment(DisplayYesNo,KeyboardDisplay)
dnl Generate_NC_ResDCommitment(KeyboardDisplay,KeyboardDisplay)

Generate_NC_InitDSendNonce(DisplayYesNo,DisplayYesNo)
dnl Generate_NC_InitDSendNonce(KeyboardDisplay,DisplayYesNo)
dnl Generate_NC_InitDSendNonce(DisplayYesNo,KeyboardDisplay)
dnl Generate_NC_InitDSendNonce(KeyboardDisplay,KeyboardDisplay)
...
dnl Generate_PE_InitDDisplay(KeyboardDisplay,KeyboardOnly)
dnl Generate_PE_InitDDisplay(DisplayOnly,KeyboardDisplay)

//Initiator Device Ask for Input
Generate_PE_InitDAskforInput(KeyboardOnly,DisplayOnly)
dnl Generate_PE_InitDAskforInput(KeyboardDisplay,DisplayOnly)
Generate_PE_InitDAskforInput(KeyboardOnly,DisplayYesNo)
Generate_PE_InitDAskforInput(KeyboardOnly,KeyboardOnly)
dnl Generate_PE_InitDAskforInput(KeyboardOnly,KeyboardDisplay)

//Responder Device Display
Generate_PE_ResDDisplay(KeyboardOnly,DisplayOnly)
dnl Generate_PE_ResDDisplay(KeyboardDisplay,DisplayOnly)
Generate_PE_ResDDisplay(KeyboardOnly,DisplayYesNo)
dnl Generate_PE_ResDDisplay(KeyboardOnly,KeyboardDisplay)
...
dnl Generate_PE_ResDAskforInput(KeyboardDisplay,KeyboardOnly)
dnl Generate_PE_ResDAskforInput(DisplayOnly,KeyboardDisplay)
...
rule InitDSendEa [color=#BBFFFF]:
    let
        Ea = f3(DHKeyI,~Na,Nb,rb,IOCapabilityI,MacAddI,MacAddR)
        LKI = f2(DHKeyI,~Na,Nb,MacAddI,MacAddR)
    in
    [
        !ResDKeySize(KeySizeRes),
        State_Init_OK(InitD,MacAddI,IOCapabilityI,OOBflagI,MITMI,MacAddR,IOCapabilityR,OOBflagR,MITMR,~skI,DHpkR,DHKeyI,ra,rb,~Na,Nb)
    ...
    --[
        ...
        Running_Init(MacAddI,MacAddR,<'LK',LKI>)
    ]->
    [
        Out(<MacAddI,MacAddR,Ea>),
        InitDSentEa(InitD,MacAddI,IOCapabilityI,OOBflagI,MITMI,MacAddR,IOCapabilityR,OOBflagR,MITMR,~skI,DHpkR,DHKeyI,ra,rb,~Na,Nb)
    ]

rule ResDSendEb [color=#FFF68F]:
    let
        LKR = f2(DHKeyR,Na,~Nb,MacAddI,MacAddR)
        Eb = f3(DHKeyR,~Nb,Na,ra,IOCapabilityR,MacAddR,MacAddI)
    ...
    --[ 
        Running_Res(MacAddR,MacAddI,<'LK',LKR>),
        Eq(Ea,f3(DHKeyR,Na,~Nb,rb,IOCapabilityI,MacAddI,MacAddR)),
        Commit_Res(MacAddR,MacAddI,<'LK',LKR>),
        Commit_Res(MacAddR,MacAddI,<'DHKey',DHKeyR>),
        FinishedRes(),
        MP(MacAddI,MacAddR),
        LK(MacAddR,MacAddI,LKR),
        FSecAuthLTK(MacAddR,MacAddI,LKR)
    ]->
    [
        Out(<MacAddR,MacAddI,Eb>),
        State_ResDAuthSta2_End(ResD,MacAddI,IOCapabilityI,OOBflagI,MITMI,MacAddR,IOCapabilityR,OOBflagR,MITMR,~skR,DHpkI,DHKeyR,ra,rb,Na,~Nb,LKR)
    ]
...
rule InitDCheck [color=#BBFFFF]:
    ...
    --[ 
        Eq(Eb,f3(DHKeyI,Nb,~Na,ra,IOCapabilityR,MacAddR,MacAddI)),
        Commit_Init(MacAddI,MacAddR,<'LK',LKI>),
        Commit_Init(MacAddI,MacAddR,<'DHKey',DHKeyI>),
        FinishedInit(),
        MP(MacAddI,MacAddR),
        LK(MacAddI,MacAddR,LKI),
        FSecAuthLK(MacAddI,MacAddR,LKI)
    ]->
    [
        State_InitDAuthSta2_End(InitD,MacAddI,IOCapabilityI,OOBflagI,MITMI,MacAddR,IOCapabilityR,OOBflagR,MITMR,~skI,DHpkR,DHKeyI,ra,rb,~Na,Nb,LKI)
    ]
...
predicates: A3(a,b,lk) <=> (All #i. Commit_Init(a,b,<'LK',lk>) @i
        ==> (Ex #j. Running_Res(b,a,<'LK',lk>) @j)
        | (Ex #k. InitDJW()@k))

predicates: A4(a,b,lk) <=> (All #i. Commit_Res(a,b,<'LK',lk>) @i
        ==> (Ex #j. Running_Init(b,a,<'LK',lk>) @j)
        | (Ex #k. ResDJW()@k))

predicates: A5(a,b,lk) <=> (All #i. Commit_Init(a,b,<'LK',lk>) @i
        ==> (Ex #j. Running_Res(b,a,<'LK',lk>) @j))

predicates: A6(a,b,lk) <=> (All #i. Commit_Res(a,b,<'LK',lk>) @i
        ==> (Ex #j. Running_Init(b,a,<'LK',lk>) @j))
...
lemma IAuthRwithLTK:
  "All I R LK. A3(I,R,LK)"

lemma RAuthIwithLTK:
  "All I R LK. A4(I,R,LK)"


lemma MITMP:
    "
    All I R #i. MP(I,R)@i ==> not (Ex LKI LKR #t1 #t2. not(A3(I,R,LKI)) & not(A4(R,I,LKR)) & K(LKI)@t1 & K(LKR)@t2)
    "

lemma LTKCP:
  "
    All I R LKI LKR #i #j. LK(I,R,LKI)@i & LK(R,I,LKR)@j ==> not (not(A3(I,R,LKI)) & not(A4(R,I,LKR)) & not(Ex #t1 #t2. K(LKI)@t1 & K(LKR)@t2))
  "

lemma SecAuthLTK:
  "
  All I R LK #i #j. FSecAuthLK(I,R,LK)@i & FSecAuthLK(R,I,LK)@j ==> (A5(I,R,LK) & A6(R,I,LK) ==> not (Ex #k. K(LK)@k))
  "
